...
const GetUserDetails = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const user = await Users.findOne({accountOwner: useraccount?._id});
        ...
        return res.status(200).json({ status: 'ok', message: 'User data fetched successfully', data: useraccount, profilePhoto: user?.profilePhoto });
    } catch (error) {
...
const UploadProfilePhoto = async (req: any, res: Response, next: NextFunction) => {
    try {
        ...
        if (!req.file) {
            ...
            return res
                .status(200)
                .json({ data: updateProfile, status: 'ok', code: 200, message: 'Profile photo updated successfully' });
